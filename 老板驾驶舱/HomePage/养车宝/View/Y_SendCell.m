...
@implementation Y_SendCell

+ (instancetype)Y_SendCellWithTableview:(UITableView *)tableview {
    
    static NSString *cellID = @"Y_SendCell";
    
    Y_SendCell *cell = [tableview dequeueReusableCellWithIdentifier:cellID];
    
    if (cell == nil) {
        
        cell = [[Y_SendCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
...
- (void)setContent {
    ...
    if ([model.Brandname isEqualToString:@""]) {
        
        self.xiaofeiLab.text = @"品牌:暂无";
        
    }else {
        
        self.xiaofeiLab.text = [NSString stringWithFormat:@"品牌:%@",model.Brandname];
        
    }
    
    if ([model.CarNo isEqualToString:@""]) {
        
        self.cuxiaoLab.text = @"车牌:暂无";
        
    }else {
        
        self.cuxiaoLab.text = [NSString stringWithFormat:@"车牌:%@",model.CarNo];
