...
@interface EditProductController ()<UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIPickerViewDataSource,UITextFieldDelegate,UIPickerViewDelegate> {
    ...
    NSMutableDictionary *imageDicCopy;
...
- (void)viewDidLoad {
    ...
    imageDicCopy = [[NSMutableDictionary alloc]initWithCapacity:0];
    ...
    addview.nameField.placeholder = self.ProName;
    
    addview.titleField.placeholder = self.ProHuoHao;
    
    if ([self.CategoryID3Name isEqualToString:@""]) {
        
        if ([self.CategoryID2Name isEqualToString:@""]) {
            
            if ([self.CategoryID1Name isEqualToString:@""]) {
                
                addview.typeField.placeholder = @"";
                
            }else {
            
                addview.typeField.placeholder = self.CategoryID1Name;
                
            }
            
        }else {
        
            addview.typeField.placeholder = self.CategoryID2Name;
            
        }
        
    }else {
    
        addview.typeField.placeholder = self.CategoryID3Name;
        
    }
    
    addview.descField.placeholder = self.ProDesc;
    
    copy2Image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.ProFengmian]]];
    
    addview.tuBiaoImageView.image = copy2Image;
    
    [imageDicCopy setValue:UIImageJPEGRepresentation(copy2Image, 1) forKey:[NSString stringWithFormat:@"fengMian"]];
...
//    
//    [imageDic setValue:UIImageJPEGRepresentation(image, 1) forKey:[NSString stringWithFormat:@"fengMian"]];
...
- (void)saveBtnClick {
    ...
    if (imageDic.count == 0) {
        ...
        imageDic = imageDicCopy;
        
    }
    
    if ([proName isEqualToString:@""] || [huoHao isEqualToString:@""]) {
